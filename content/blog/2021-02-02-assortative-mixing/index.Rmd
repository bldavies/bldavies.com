...
keys = paste(c('Positively', 'Negatively'), 'sorted network')

set.seed(0)
Gp = sample_ppm(memb, a, b) %>%
  as_tbl_graph() %>%
  mutate(memb = memb,
         key = keys[1])
Gn = sample_ppm(memb, b, a) %>%
  as_tbl_graph() %>%
  mutate(memb = memb,
         key = keys[2])
```
...
The network on the left has edge frequencies $(p,q)=(`r a`,`r b`)$ and assortativity coefficient $r=`r sprintf('%.2f', assortativity_nominal(Gp, memb))`$; the network on the right has edge frequencies $(p,q)=(`r b`,`r a`)$ and assortativity coefficient $r=`r sprintf('%.2f', assortativity_nominal(Gn, memb))`$.
...
bind_graphs(Gp, Gn) %>%
  ...
  geom_node_point(aes(col = factor(memb)), show.legend = F) +
  facet_nodes(~key, scales = 'free') +
  labs(title = paste('Two networks with', n1 + n2, 'nodes, split evenly between types'),
       subtitle = 'Nodes with different colors have different types')
```
...
For example, the left-hand network in the figure above has $\rho=`r round(assortativity_degree(Gp, directed = F), 2)`$: although nodes are sorted strongly by color, they are approximately unsorted by degree because the planted partition model from which the network is generated has no mechanism for connecting high-degree nodes.
...
N = 1e3
set.seed(0)
res = sapply(1:N, function(i) assortativity_degree(sample_degseq(degree(Gp))))

tibble(res = res) %>%
  ggplot(aes(res)) +
  geom_density() +
  geom_vline(xintercept = assortativity_degree(Gp), lty = 'dashed') +
  labs(x = 'Degree assortativity',
       y = 'Density',
       title = paste('Distribution of degree assortativities\nacross', N, 'degree-preserving randomizations'),
       subtitle = 'Dashed line represents original network')
```
