...
```{r get_epv}
get_epv = function(p, a, b) {
  n = p * (1 - p) * (a * (1 - a) * (1 - p) + b * (1 - b) * p)
  d = ((1 - a) * (1 - p) + b * p) * (a * (1 - p) + (1 - b) * p)
  return(n / d)
}
```

```{r convexity}
crossing(p = c(0.5, 0.7, 0.9), eps = seq(0, 0.5, by = 0.01)) %>%
  mutate(epv = map2_dbl(p, eps, ~get_epv(.x, .y, .y))) %>%
  ggplot(aes(eps, epv)) +
  geom_line(aes(lty = paste('p =', p))) +
  labs(x = 'False positive/negative rate (\u03b5)',
       y = 'Expected posterior variance',
       title = 'Expected posterior variance when \u03b1 = \u03b2 = \u03b5',
       subtitle = 'Signals are more informative when \u03b5 is farther from 0.5',
       lty = 'Prior') +
...
```{r contours}
crossing(p = c(0.5, 0.7, 0.9), a = seq(0, 0.5, by = 0.01), b = seq(0, 0.5, by = 0.01)) %>%
  mutate(epv = pmap_dbl(list(p = p, a = a, b = b), get_epv)) %>%
  filter(!is.na(epv)) %>%
  ggplot(aes(a, b)) +
  geom_contour_filled(aes(z = epv), show.legend = F) +
  labs(x = 'False positive rate (\u03b1)',
       y = 'False negative rate (\u03b2)',
       title = 'Expected posterior variance',
       subtitle = 'Darker regions represent larger values') +
  facet_wrap(~paste('p =', p)) +
