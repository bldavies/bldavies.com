...
About `r scales::comma(round(C[2, 2] / 1e3) * 1e3)` graduates hold degrees in `r fields$field_desc[2]`, nearly `r scales::comma(round(C[2, 4] / 1e3) * 1e3)` of which also hold degrees in `r fields$field_desc[4]`.
`r fields$field_desc[3]` attracts about as many graduates as `r fields$field_desc[5]`, but no respondents in the PUMS data reported studying both.
...
```{r, echo = FALSE}
tibble(field1 = rep(fields$field_desc, nrow(C)),
       field2 = rep(fields$field_desc, each = nrow(C))) %>%
  filter(as.vector(upper.tri(C) & C > 0)) %>%
  cbind(similarities) %>%
  select(-Jaccard) %>%
  mutate_if(is.numeric, function(x) min_rank(-x)) %>%
  arrange(Dice) %>%
  top_n(10, -Dice) %>%
  knitr::kable(col.names = c('Field 1', 'Field 2', 'Dice/Jacc. rank', 'Ochiai rank', 'Overlap rank', 'Co-occ. rank'))
```
...
```{r, echo = FALSE}
tibble(Field = fields$field_desc) %>%
  cbind(pageranks) %>%
  mutate_if(is.numeric, function(x) min_rank(-x)) %>%
  mutate(Size = min_rank(-diag(C))) %>%
  arrange(Dice) %>%
  top_n(10, -Dice) %>%
  `names<-`(c('Field', paste(names(.)[2:7], 'rank'))) %>%
  knitr::kable()
```
...
```{r, echo = FALSE}
save.image('../../data/college-degrees/similarity-measures.RData')
```
