...
```{r plot}
plot_df = crossing(b = c(0.1, 0.5, 1), p = seq(0, 0.5, by = 0.01), alpha = c(0.5, 1, 2)) %>%
  mutate(UL = 0.5  * b,
         UH = 0.5 * (1 + p) * b + 0.5 * (1 + p) * ((1 + 2 * p) / (1 + p)) ^ alpha + 0.5 * (1 - p) * ((1 - 2 * p) / (1 - p)) ^ alpha - 1,
         D = UH - UL)

plot_df %>%
  ggplot(aes(p, D)) +
  geom_line(aes(col = paste('b =', b))) +
  facet_wrap(~paste('\u03b1 =', alpha)) +
  labs(title = 'Incentives to self-sabotage',
       subtitle = 'The difference D depends on the parameters p, b, and \u03b1',
       col = NULL) +
