...
set.seed(0)
sim = tibble(x = rnorm(n),
       y = (x + rnorm(n)) / 2,
       u = rnorm(n),
       y_bar = y + u,
       y_hat = as.vector(x %*% solve(t(x) %*% x) %*% t(x) %*% y_bar))

sim_mse = sim %>%
  summarise(bar = mean((y_bar - y) ^ 2),
            hat = mean((y_hat - y) ^ 2))
```
...
The modeled predictions are far more accurate: they have an MSE of `r round(sim_mse$hat, 2)`, whereas my raw predictions have an MSE of `r round(sim_mse$bar, 2)`.
...
```{r example}
sim %>%
  select(y, `Raw predictions` = y_bar, `Modeled predictions` = y_hat) %>%
  gather(key, value, -y, factor_key = T) %>%
  ggplot(aes(y, value)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0, linetype = 'dashed') +
  labs(x = 'True value',
       y = NULL,
       title = 'Models of human predictions can out-perform the humans\non which those models are based',
       subtitle = 'Dots closer to dashed line represent more accurate predictions') +
  facet_wrap(~key)
```
...
This condition holds in the example plotted above: both $\sigma_u^2$ and $\sigma_y^2$ equal unity, but $\rho_{xy}=`r round(cor(sim$x, sim$y), 2)`$ is much larger in absolute value than $\rho_{ux}=`r round(cor(sim$x, sim$u), 2)`$.
