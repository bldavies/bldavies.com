...
```{r functions}
get_posterior = function(signal, prior, x) {
  if (x == 1) {
    signal
  } else if (signal == 1) {
    (1 + x) * prior / ((1 + x) * prior + (1 - x) * (1 - prior))
  } else {
    (1 - x) * prior / ((1 - x) * prior + (1 + x) * (1 - prior))
  }
}

get_loss = function(state, signal, prior, x, a = 1) {
  ifelse(state == 0, a, 1) * (state - get_posterior(signal, prior, x)) ^ 2
}

get_objective = function(x, prior, mc = 0, a = 1) {
  res = 0
  res = res + ((1 + x) * (1 - prior) / 2) * get_loss(0, 0, prior, x, a)
  res = res + ((1 - x) * (1 - prior) / 2) * get_loss(0, 1, prior, x, a)
  res = res + ((1 - x) * prior / 2) * get_loss(1, 0, prior, x, a)
  res = res + ((1 + x) * prior / 2) * get_loss(1, 1, prior, x, a)
  res + mc * x
}

get_choke_price = function(prior, w = 1) {
  f = function(mc) get_objective(0, prior, mc, w) - get_objective(1, prior, mc, w)
  uniroot(f, c(0, 2 * w))$root
}
```
...
```{r objectives, fig.height = 4}
dat = crossing(prior = c(0.5, 0.7, 0.9), x = 0.05 * (0:20), mc = 0.1 * (0:3)) %>%
  mutate(objective = pmap_dbl(list(x = x, prior = prior, mc = mc), get_objective))

dat %>%
  ggplot(aes(x, objective)) +
  geom_line(aes(lty = factor(prior))) +
  facet_wrap(~paste('c =', mc)) +
  labs(title = 'Objectives under different priors and marginal costs',
       subtitle = 'I optimally seek all the truth or none of it',
       x = 'Signal quality x',
       y = 'Objective f(x)',
       lty = 'Prior') +
  guides(lty = guide_legend(title.hjust = 1)) +
...
```{r choke-prices}
dat = crossing(prior = 0.01 * (50:100)) %>%
  mutate(choke_price = map_dbl(prior, get_choke_price))

dat %>%
  ggplot(aes(prior, choke_price)) +
  geom_line() +
  labs(title = 'Choke prices under different priors',
       subtitle = 'The more ideological I am, the cheaper the truth must be to seek it',
       x = 'Prior',
       y = 'Choke price c*') +
  coord_cartesian(clip = 'off') +
  scale_x_continuous(expand = c(0, 0), labels = c(0.5, 0.6, 0.7, 0.8, 0.9, 1)) +
  scale_y_continuous(expand = c(0, 0))
```
...
```{r choke-prices-weighted}
dat = crossing(prior = 0.01 * (50:100), w = c(1, 3, 5)) %>%
  mutate(choke_price = map2_dbl(prior, w, get_choke_price))

dat %>%
  ggplot(aes(prior, choke_price)) +
  geom_line(aes(lty = factor(w)), show.legend = F) +
  geom_text(data = filter(dat, prior == 0.5), aes(y = choke_price - 0.1 / (w + 1), label = paste('w =', w)), hjust = 0, nudge_x = 0.01) +
  labs(title = 'Choke prices under different priors and error weights',
       subtitle = 'c* is non-monotonic in my prior when the error weight w is large',
       x = 'Prior',
       y = 'Choke price c*') +
  coord_cartesian(clip = 'off') +
  scale_x_continuous(expand = c(0, 0), labels = c(0.5, 0.6, 0.7, 0.8, 0.9, 1)) +
