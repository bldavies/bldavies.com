...
```{r simulations}
# Define function for generating preference rank matrix
get_rankings = function(n, rho) {
  W = matrix(rep(runif(n), each = n), nrow = n)  # Common component
  X = matrix(runif(n * n), nrow = n)  # Idiosyncratic component
  U = rho * W + (1 - rho) * X
  t(apply(U, 1, function(x) rank(-x)))
}

# Define function for simulating one realization of the random preference ranks
# and applying the GS algorithm to them
simulate = function(n, rho) {
  
  # Initialize preference orderings
  RA = get_rankings(n, rho)
  RB = get_rankings(n, rho)
  
  # Run Gale-Shapley algorithm
  allocs = rep(0, n)
  props = rep(0, n)
  while (sum(allocs == 0 & props < n) > 0) {
    a = min(which(allocs == 0 & props < n))
    props[a] = props[a] + 1
    b = which(RA[a, ] == props[a])
    if (!b %in% allocs) {
      allocs[a] = b
    } else if (RB[b, a] < RB[b, which(allocs == b)]) {
      allocs[which(allocs == b)] = 0
      allocs[a] = b
    }
  }
...
N = 1e3
set.seed(0)
sims = crossing(rho = 0.1 * (0:10), realization = 1:N) %>%
  mutate(proposals = sapply(rho, function(x) simulate(n, x)))
```
...
The chart below shows how the number of proposals required by the GS algorithm covaries with $\rho$ when $n=`r n`$.
...
sims %>%
  group_by(rho) %>%
  mutate(mean = mean(proposals),
         ci_lower = quantile(proposals, (1 - coverage) / 2),
         ci_upper = quantile(proposals, (1 + coverage) / 2)) %>%
  ggplot(aes(rho, mean)) +
  geom_ribbon(aes(ymin = ci_lower, ymax = ci_upper), alpha = 0.25) +
  geom_line() +
  labs(x = 'rho',
       y = 'Number of proposals',
       title = 'More homogeneous preferences lead to more proposals',
       subtitle = paste('Means and 95% CIs across', format(N, big.mark = ','), 'simulated matchings of', n, 'pairs')) +
